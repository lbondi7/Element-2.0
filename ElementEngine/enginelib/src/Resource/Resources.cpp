...
template<> struct std::hash<Element::Vertex> {
    size_t operator()(Element::Vertex const& vertex) const {
...
bool Element::Resources::LoadMeshData(const char* file, Element::Mesh& mesh)
{
    ...
    for (const auto& shape : shapes) {
        ...
        for (const auto& index : shape.mesh.indices) {
            ...
            if (uniqueVertices.count(vertex) == 0) {
                ...
                mesh.max.x = std::max(mesh.max.x, vertex.pos.x);
                mesh.max.y = std::max(mesh.max.y, vertex.pos.y);
                mesh.max.z = std::max(mesh.max.z, vertex.pos.z);

                mesh.min.x = std::min(mesh.min.x, vertex.pos.x);
                mesh.min.y = std::min(mesh.min.y, vertex.pos.y);
                mesh.min.z = std::min(mesh.min.z, vertex.pos.z);
            }
...
bool Element::Resources::LoadMeshData(const std::string& file, Mesh& mesh)
{
    return LoadMeshData(file.c_str(), mesh);
}
...
Element::Mesh *Element::Resources::mesh(const std::string &name, State state) {
    ...
    MeshData data;
    if(!LoadMeshData(name.c_str(), mesh)){
        if(state == State::STATIC)
