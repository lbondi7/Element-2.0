...
void Element::ImGUI::initResources(VkRenderPass renderPass)
{
    ...
    Debugger::get().log("Imgui Image Staging Buffer", Colour::BRIGHT_BLUE);
...
void Element::ImGUI::updateBuffers()
{
	...
	if ((vertexBuffer.m_buffer == VK_NULL_HANDLE) || (vertexCount != imDrawData->TotalVtxCount)) {
        ...
        Debugger::get().log("Imgui Vertex Buffer", Colour::BRIGHT_BLUE);
	...
	if ((indexBuffer.m_buffer == VK_NULL_HANDLE) || (indexCount != imDrawData->TotalIdxCount)) {
		indexBuffer.Unmap();
		indexBuffer.Destroy();
        Debugger::get().log("Imgui Index Buffer", Colour::BRIGHT_BLUE);
